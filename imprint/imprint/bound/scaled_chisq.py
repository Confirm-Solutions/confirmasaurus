...
def A_secant(n, df, theta, v, q):
    ...
    df: a scalar or (d,)-vector of degrees of freedom for each arm.
    theta:      a (d,)-array where theta1[i] is the
                natural parameter of arm i.
    v:          a (d,)-array of displacements.
    """
    return -0.5 * jnp.sum(df * n * jnp.log(1 + q * v / theta)) / q


class BaseTileQCPSolver:
    def __init__(self, n, df, m=1, M=1e7, tol=1e-5, eps=1e-6):
        self.n = n
        self.df = df
        self.min = m
        self.max = M
...
def tilt_bound_fwd_tile(
    ...
    def _expo(v):
        expo = A_secant(n, df, theta_0, v, q)
...
def tilt_bound_bwd_tile(
    ...
    def _expo(v):
        slope_diff = A_secant(n, df, theta_0, v, q)
...
class ScaledChiSqBound:
    ...
    def get_backward_bound(family_params):
        n = family_params["n"]
        df = family_params["df"]
        bwd_solver = TileBackwardQCPSolver(n, df)
    ...
    def get_forward_bound(family_params):
        n = family_params["n"]
        df = family_params["df"]
        fwd_solver = TileForwardQCPSolver(n, df)

        def forward_bound(f0, theta0, vertices):
            vs = vertices - theta0
            q_opt = fwd_solver.solve(theta0, vs, f0)
            return tilt_bound_fwd_tile(q_opt, n, df, theta0, vs, f0)
