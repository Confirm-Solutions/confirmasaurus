...
def pytest_configure(config):
    config.addinivalue_line("markers", "modal_unsafe: mark test as unsafe to run on Modal.")
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.getoption("--run-modal-safe"):
        skip_modal_unsafe = pytest.mark.skip(reason="--run-modal-safe prevented running this test")
        for item in items:
            if "modal_unsafe" in item.keywords:
                item.add_marker(skip_modal_unsafe)
    elif config.getoption("--run-modal-unsafe"):
        skip_modal_safe = pytest.mark.skip(reason="--run-modal-unsafe prevented running this test")
        for item in items:
...
def pytest_addoption(parser):
    ...
    """
    parser.addoption(
        "--run-modal-safe", action="store_true", default=False, help="run only Modal-safe tests"
    )
    parser.addoption(
        "--run-modal-unsafe", action="store_true", default=False, help="run only Modal-unsafe tests"
    )
