...
class Lewis45:
    def __init__(
        ...
        self.p_pinned = dict(sig2=self.sig2_rule.pts, theta=None)
        self.n_sig2 = self.sig2_rule.pts.shape[0]
        self.custom_ops = berry.optimized(self.sig2_rule.pts, n_arms=n_arms).config(
            opt_tol=1e-3
        )
        self.jit_laplace_logpost = jax.jit(self.custom_ops.laplace_logpost)
        self.x0 = np.zeros((self.n_sig2, n_arms), dtype=self.dtype)
    ...
    def posterior_sigma_sq(self, data):
        ...
        """
        n_arms, _ = data.shape
        logpost, x_max, hess, iters = self.jit_laplace_logpost(
            self.x0, self.p_pinned, data
        )
        post = inla.exp_and_normalize(logpost, self.sig2_rule.wts, axis=0)
    ...
    def stage_1(self, p, keys):
        ...
        def loop_body(i, args):
            data, pr_best, non_futile_idx, n_non_futile = args
            ...
            return data, pr_best, non_futile_idx, n_non_futile

        data, pr_best, non_futile_idx, n_non_futile = jax.lax.fori_loop(
            0, n_interims, loop_body, (data, pr_best, non_futile_idx, 0)
        )
        return data, n_non_futile, non_futile_idx, pr_best
    ...
    def single_sim(self, p, keys):
        ...
        # Stage 1:
        data, n_non_futile, non_futile_idx, pr_best = self.stage_1(
            p=p,
            keys=keys[:-1],
        )
        return n_non_futile
