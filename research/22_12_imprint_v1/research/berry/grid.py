...
def make_cartesian_gridpts(n_theta_1d, lower, upper):
    n_arms = lower.shape[0]
    theta1d = [
        np.linspace(lower[i], upper[i], 2 * n_theta_1d + 1)[1::2] for i in range(n_arms)
    ]
...
class Grid:
    ...
    def n_tiles_per_pt(self):
        _, out = np.unique(self.grid_pt_idx, return_counts=True)
        return out
